...
1. Open `DSF_Scholarship_Editor.html`, make your changes, and export the updated JSON.
2. Rename the export to `DSF_scholarships.json`.
3. Put that file, both HTML files, and `update_portal.py` in one folder.
4. Run:
   ```
   python update_portal.py
   ```
5. Upload the two updated HTML files to this repository. The live site redeploys automatically within a minute.
...
Hosted on [Vercel](https://vercel.com). Every commit to the `main` branch redeploys the site automatically — there is nothing to build or configure.
